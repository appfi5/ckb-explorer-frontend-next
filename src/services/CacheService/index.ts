...
import { type PersistenceService, persistenceService } from "../PersistenceService";
import type { MutexAttr } from "@/utils/typescript";
import { env } from "@/env";

export const NAMESPACE_CACHE = "cache";

type ExpireOptions = MutexAttr<
  { expireTime?: number },
...
export class CacheService {
  constructor(private persistenceService: PersistenceService) {
    this.getAllKeys().forEach((key) =>
      this.removeIfExpired(
        key,
        this.persistenceService.get<CacheData<unknown>>(key),
      ),
    );
  }
  ...
  get<T = unknown>(key: string, defaultValue?: T): T | undefined {
    const keyInChain = `${NAMESPACE_CACHE}_${env.NEXT_PUBLIC_CHAIN_TYPE}_${key}`;
    const cacheInChain = this.persistenceService.get<CacheData<T>>(keyInChain);
    if (
      cacheInChain != null &&
      !this.removeIfExpired(keyInChain, cacheInChain)
    ) {
    ...
    const cacheAllChain =
      this.persistenceService.get<CacheData<T>>(keyAllChain);
    if (
      cacheAllChain != null &&
      !this.removeIfExpired(keyAllChain, cacheAllChain)
    ) {
  ...
  set<T = unknown>(key: string, value: T, opts: CacheOptions = {}): boolean {
    const {
      expireTime = ONE_DAY_MILLISECOND * 7,
      expireAt,
      networkTypeAgnostic,
    } = opts;

    const keyPrefix = networkTypeAgnostic
      ? NAMESPACE_CACHE
      : `${NAMESPACE_CACHE}_${env.NEXT_PUBLIC_CHAIN_TYPE}`;
  ...
  getAllKeys() {
    return this.persistenceService
      .getAllKeys()
      .filter((key) => key.startsWith(`${NAMESPACE_CACHE}_`));
  }
  ...
  removeIfExpired(key: string, cacheData: CacheData<unknown> | undefined) {
    ...
    if (cacheData.expireAt === undefined || cacheData.expireAt > Date.now())
      return false;
