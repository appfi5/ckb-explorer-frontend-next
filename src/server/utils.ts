...
* 
*/
import { requestAPI as request, type RequestConfig } from "@/utils/request";
...
export class Interceptors<Group extends Record<any, [any, any]>> {
  private requestObserverMap = new Map<keyof Group, RequestInterceptor>();
  private responseObserverMap = new Map<keyof Group, ResponseInterceptor>();
  ...
  public add<IPath extends keyof Group>(key: IPath,
    ...
    if (requestInterceptor) this.requestObserverMap.set(key, requestInterceptor);
    if (this.responseObserverMap.has(key)) {
      console.warn(`Response Interceptors: ${key as string} is exist`);
    }
    if (responseInterceptor) this.responseObserverMap.set(key, responseInterceptor);
    return this;
  }
