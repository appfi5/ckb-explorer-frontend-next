...
export const useFormatConfirmation = () => {
  ...
  return (confirmation: number) => {
    ...
    return `${Math.max(confirmation, 0)} ${t("address.confirmation")}`;
  };
};
...
// export const matchScript = (
//   contractHash: string,
//   hashType?: string,
// ): ContractHashTag | undefined => {
//   if (isMainnet()) {
...
export const getBtcUtxo = (script: Script) => {
  ...
  const [index, txid] = [
    script.args.slice(2, d),
    script.args.slice(d, d + TXID_BYTE_SIZE * 2),
  ].map((v) => v.match(/\w{2}/g)?.reverse().join(""));
  return { txid, index };
};
...
export const handleRedirectFromAggron = () => {
  ...
  const testnetNameRegexp = new RegExp(
...
export const handleNftImgError = (
  e: SyntheticEvent<HTMLImageElement, Event>,
) => {
  const img = e.currentTarget;
  const { assetType } = img.dataset;
  if (assetType === "DOB") {
    e.currentTarget.src = "/assets/dob-cover.svg";
    e.currentTarget.className = `${img.className} text-primary`;
    return;
  }
  e.currentTarget.src = "/assets/nft_cover.svg";
};

export const handleFtImgError = (
  e: SyntheticEvent<HTMLImageElement, Event>,
) => {
  e.currentTarget.src = "/assets/ft_fallback_icon.png";
};
...
export const isDaoDepositCell = (cellType: Cell["cellType"]) =>
  cellType === "nervos_dao_deposit";

export const isDaoWithdrawCell = (cellType: Cell["cellType"]) =>
  cellType === "nervos_dao_withdrawing";

export const isDaoCell = (cellType: Cell["cellType"]) =>
  isDaoDepositCell(cellType) || isDaoWithdrawCell(cellType);
...
export const isMac = () => {
  return isClient ? navigator.userAgent.includes("Mac") : false;
};
