...
export const handleLogGroupAxis = (
  value: BigNumber | string | number,
  suffix?: string,
) => {
  const bigValue =
...
export const handleStepGroupAxis = (
  value: BigNumber | string | number,
  step?: number,
  suffix?: string,
) => {
...
export const getFeeRateSamples = (
  ...
  const sampleCount = Math.max(
    SAMPLES_MIN_COUNT,
    Number.isNaN(TPM) ? 0 : Math.floor(TPM) * 10,
  );
...
export function assertIsArray<T>(value: T | T[]): asserts value is T[] {
  if (!Array.isArray(value)) {
    throw new Error(
      `Value is expected to be an array, but got a ${typeof value}`,
    );
  }
}
...
export const assertSerialsDataIsString: (
  value: unknown,
) => asserts value is { data: string } = (value: unknown) => {
  if (
    typeof value !== "object" ||
    value === null ||
    ...
    throw new Error(
      `Value is expected to be an string, but got a ${typeof (value as any).data}`,
    );
  }
};
...
export const variantColors = (
  ...
  function adjustColor(color: string, factor: number) {
    ...
    const r = Math.min(
      255,
      Math.max(0, parseInt(hex.substring(0, 2), 16) + factor),
    );
    const g = Math.min(
      255,
      Math.max(0, parseInt(hex.substring(2, 4), 16) + factor),
    );
    const b = Math.min(
      255,
      Math.max(0, parseInt(hex.substring(4, 6), 16) + factor),
    );
