...
const toChangeData = (statisticLiquidity: ChartItem.Liquidity[]) => statisticLiquidity.map((item) => ({
  ...item,
  daoDeposit: new BigNumber(item.circulatingSupply)
    .minus(new BigNumber(item.liquidity))
    .toFixed(2),
}));
...
const useOption = (
  ...
  return {
    ...
    dataset: {
      source: toChangeData(statisticLiquidity).map(data => [
...
const toCSV = (statisticLiquidity: ChartItem.Liquidity[]) =>
  statisticLiquidity
    ? toChangeData(statisticLiquidity).map(data => [
