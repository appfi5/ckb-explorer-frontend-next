...
import DownArrowIcon from "./down_arrow.svg?component";
...
type CardCellInfo$WithSlot<S extends LayoutSlot = LayoutSlot> = {
  slot: S;
  cell: CardCellInfo$WithoutSlot;
};
export type CardCellInfo<S extends LayoutSlot = LayoutSlot> =
  | CardCellInfo$WithSlot<S>
  | CardCellInfo$WithoutSlot;

function isCardCellInfoWithSlot(
  info: CardCellInfo,
): info is CardCellInfo$WithSlot {
...
interface CardCellsLayoutProps$LeftSingleRight
  extends CardCellsLayoutProps$Common {
...
type CardCellsLayoutProps = ComponentProps<"div"> &
  (
    | CardCellsLayoutProps$LeftRight
    | CardCellsLayoutProps$LeftSingleRight
    | CardCellsLayoutProps$List
  );
...
> = ({ type, cells, displayCount }) => {
  ...
  const dataCss = "md:bg-[#F5F9FB] md:p-[20px] md:rounded-[16px] dark:md:bg-[#303030]";
...
const CardCellsLayout$List: FC<
  { displayCount: number } & CardCellsLayoutProps$List
> = ({ cells, displayCount }) => {
  const finalCells = useMemo(
    () =>
      cells
        .slice(0, displayCount)
        .map((info) => (isCardCellInfoWithSlot(info) ? info.cell : info)),
    [cells, displayCount],
  );

  return <div className={`${styles.list} md:bg-[#F5F9FB] md:p-[20px] md:rounded-[16px]`}>{finalCells.map(renderCell)}</div>;
};
...
export const CardCellsLayout: FC<CardCellsLayoutProps> = ({
  ...
  const displayCount =
    isMobile && !isExpanded ? defaultDisplayCountInMobile : Infinity;

  return (
    <div
      {...elProps}
      className={`${styles.cardCellsLayout} ${elProps.className}`}
    >
      {(type === "left-right" || type === "leftSingle-right") && (
        <CardCellsLayout$LeftRightOrLeftSingleRight
          type={type}
          cells={cells}
      ...
      {showExpandCtl && (
        <div
          ...
          onPointerDown={() => expandCtl.toggle()}
          role="button"
          tabIndex={0}
        >
          <DownArrowIcon
            className={classNames({ [styles.isExpanded ?? ""]: isExpanded })}
          />
        </div>
      )}
    </div>
