...
import { CardCellsLayout } from "./CardCellsLayout";
...
export function CardList<T>({
  ...
  return (
    <div
      {...elProps}
      className={classNames(styles.cardList, elProps.className)}
    >
      {dataSource.map((data, index) => (
        <Card key={getDataKey?.(data, index) ?? index} {...cardProps} className={classNames("px-4", cardProps?.className)}>
...
interface CardListWithCellsListProps<T>
  extends Omit<CardListProps<T>, "cardContentRender"> {
  cells: CardCellFactory<T>[];
}

export function CardListWithCellsList<T>({
  cells,
  ...props
}: CardListWithCellsListProps<T>) {
  return (
    <CardList
      {...props}
      cardContentRender={(data) => (
